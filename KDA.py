...
# --- CÁLCULO DEL ICHIMOKU CLOUD ---
# Parámetros estándar
period_conversion = 9   # Tenkan-sen
period_base = 26        # Kijun-sen
period_span_b = 52      # Senkou Span B
displacement = 26       # Desplazamiento de las líneas líderes

# Cálculo del Tenkan-sen (línea de conversión)
data['Tenkan_sen'] = (data['High'].rolling(window=period_conversion).max() + data['Low'].rolling(window=period_conversion).min()) / 2
# Cálculo del Kijun-sen (línea base)
data['Kijun_sen'] = (data['High'].rolling(window=period_base).max() + data['Low'].rolling(window=period_base).min()) / 2
# Líneas líderes (Senkou Span A y B)
data['Senkou_span_A'] = ((data['Tenkan_sen'] + data['Kijun_sen']) / 2).shift(displacement)
data['Senkou_span_B'] = ((data['High'].rolling(window=period_span_b).max() + data['Low'].rolling(window=period_span_b).min()) / 2).shift(displacement)
# Chikou Span (retardado 26 períodos)
...
# Señal ICHIMOKU: cruce alcista del Tenkan_sen sobre el Kijun_sen
cond_Ichimoku = (data['Tenkan_sen'] > data['Kijun_sen']) & (data['Tenkan_sen'].shift(1) <= data['Kijun_sen'].shift(1))
...
# Cambiamos el nombre del archivo para no sobrescribir los existentes
excel_file = 'resultados_indicadores_nuevo.xlsx'
...
print("\nLos resultados se han guardado en 'resultados_indicadores_nuevo.xlsx'.")
...
# Cambiamos el nombre del archivo para no sobrescribir los existentes
historico_file = 'historico_tickers_nuevo.xlsx'
...
print("\nLa tabla de rendimientos mensuales se ha guardado en 'historico_tickers_nuevo.xlsx'.")
